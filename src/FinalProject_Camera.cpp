    ...
    for (auto itr = detector_type_vector.begin(); itr != detector_type_vector.end(); ++itr)
    {
        ...
        for(auto itr2= decriptor_type_vector.begin(); itr2 != decriptor_type_vector.end(); ++itr2)
        {
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
            {
                ...
                if(bVis)
                {
                    show3DObjects((dataBuffer.end()-1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(1000, 1000),imgNumber.str(), false);
                }
                bVis = false;
                    ...
                    for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                    {
                        ...
                        if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                        {
                            ...
                            if (bVis)
                            {
                                ...
                                cv::imwrite("../output/finalImage_"+detectorType+"_"+descriptorType+"_"+imgNumber.str()+".jpg", visImg);
